...
export default function MessageBubble({
    ...
    const cleanText =
        (msg.text || "")
            .replace(
                /^\[(image|video|audio|document)\s+message\]\s*/i,
                ""
            )
                    ...
                    <div className="mx-2 mt-2 px-3 py-2 bg-black/5 dark:bg-white/10 border-l-4 border-primary rounded flex gap-2">
                        <div className="flex-1 min-w-0">
                            <div className="text-xs font-semibold text-primary mb-1">
                                {repliedMessage.sender === "executive" ? "You" : conversation.companyName}
                            </div>
                            <div className="text-xs text-muted-foreground line-clamp-2">
                                ...
                                {!repliedMessage.text && repliedMessage.caption && repliedMessage.caption}
                                {/* If no text/caption but has media, show media type */}
                                {!repliedMessage.text && !repliedMessage.caption && repliedMessage.mimeType?.startsWith("image/") && "📷 Photo"}
                                {!repliedMessage.text && !repliedMessage.caption && repliedMessage.mimeType?.startsWith("video/") && "🎥 Video"}
                                {!repliedMessage.text && !repliedMessage.caption && repliedMessage.mimeType?.startsWith("audio/") && "🎵 Audio"}
                                {/* Fallback */}
                                {!repliedMessage.text && !repliedMessage.caption && !repliedMessage.mimeType && "Message"}
                            </div>
                        </div>
                        {/* Media Thumbnail */}
