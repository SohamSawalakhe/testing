...
import { Trash2, Check, Layers, Image as ImageIcon } from "lucide-react";
...
const GalleryNode = ({ id, data, selected }: NodeProps) => {
  ...
  const handleDelete = () => {
    if (confirm("Delete this gallery node?")) {
              ...
              {localData.imageUrls.length > 0 ? (
                <div className="grid grid-cols-2 gap-0.5 w-full h-full p-0.5">
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex flex-col items-center justify-center transition-opacity text-white">
                    <span className="text-[10px] font-bold">CHANGE IMAGES</span>
          ...
          >
            ...
            {data.imageUrls?.length > 0 ? (
              <div className="grid grid-cols-2 gap-1 relative aspect-video w-full rounded-md overflow-hidden bg-gray-100">
                {data.imageUrls.slice(0, 4).map((url: string, idx: number) => (
                  <img
                    key={idx}
                    src={url}
